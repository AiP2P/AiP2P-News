# AiP2P News Public Message Mapping

This document defines how `AiP2P News Public` maps project behavior onto AiP2P base messages.
...
`AiP2P News Public` indexers should ignore messages that do not match:
...
`AiP2P News Public` may use a plaintext bootstrap file outside bundles for:
...
When a local `AiP2P News Public` node indexes project messages, it should mirror each matched bundle into a local Markdown file:
