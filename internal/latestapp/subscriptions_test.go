...
func TestApplySubscriptionRulesFiltersByTopicAndCarriesReplies(t *testing.T) {
	...
	index := buildIndex([]Bundle{postWorld, replyWorld, postTech}, "aip2p.news")
	filtered := ApplySubscriptionRules(index, "aip2p.news", SubscriptionRules{Topics: []string{"energy"}})
...
func TestApplySubscriptionRulesReservedAllTopicShowsEverything(t *testing.T) {
	...
	index := buildIndex([]Bundle{postWorld, postTech}, "aip2p.news")
	filtered := ApplySubscriptionRules(index, "aip2p.news", SubscriptionRules{Topics: []string{"all"}})
...
func TestApplySubscriptionRulesFiltersByMaxAgeDays(t *testing.T) {
	...
	index := buildIndex([]Bundle{fresh, stale}, "aip2p.news")
	filtered := ApplySubscriptionRules(index, "aip2p.news", SubscriptionRules{Topics: []string{"all"}, MaxAgeDays: 1})
...
func TestApplySubscriptionRulesFiltersByMaxBundleMB(t *testing.T) {
	...
	index := buildIndex([]Bundle{small, large}, "aip2p.news")
	filtered := ApplySubscriptionRules(index, "aip2p.news", SubscriptionRules{Topics: []string{"all"}, MaxBundleMB: 10})
...
func TestApplySubscriptionRulesFiltersByMaxItemsPerDay(t *testing.T) {
	...
	index := buildIndex([]Bundle{first, second}, "aip2p.news")
	filtered := ApplySubscriptionRules(index, "aip2p.news", SubscriptionRules{Topics: []string{"all"}, MaxItemsPerDay: 1})
