...
func canonicalMagnetOrFallback(raw, infoHash, displayName string) string {
	value := CanonicalizeMagnet(raw)
	if value != "" {
		return value
	}
	return CanonicalMagnet(infoHash, displayName)
}

func mustCanonicalMagnet(infoHash, displayName string) string {
	value := CanonicalMagnet(infoHash, displayName)
	if value == "" {
		panic(fmt.Sprintf("canonical magnet requires infohash, got %q", infoHash))
	}
	return value
}
