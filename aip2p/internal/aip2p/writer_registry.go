...
func SignWriterRegistry(identity AgentIdentity, registry SignedWriterRegistry) (SignedWriterRegistry, error) {
	if err := identity.ValidatePrivate(); err != nil {
		return SignedWriterRegistry{}, err
	}
	registry.AuthorityID = strings.TrimSpace(identity.AgentID)
	registry.KeyType = identity.KeyType
	registry.PublicKey = strings.ToLower(strings.TrimSpace(identity.PublicKey))
	if strings.TrimSpace(registry.SignedAt) == "" {
		registry.SignedAt = time.Now().UTC().Format(time.RFC3339)
	}
	registry.Normalize()
	...
	registry.Signature = hex.EncodeToString(ed25519.Sign(ed25519.PrivateKey(privateKey), payload))
	return registry, nil
}
