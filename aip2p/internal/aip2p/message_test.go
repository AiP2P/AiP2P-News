...
func TestBuildAndLoadMessage(t *testing.T) {
	...
	if gotMsg.Extensions["project"] != "aip2p.news" {
		t.Fatalf("extensions project = %v, want aip2p.news", gotMsg.Extensions["project"])
	}
}
...
func TestBuildAndLoadSignedMessage(t *testing.T) {
	...
	dir := t.TempDir()
	identity, err := NewAgentIdentity("news/world-01", "agent://demo/alice", time.Date(2026, 3, 14, 9, 0, 0, 0, time.UTC))
	if err != nil {
	...
	msg, body, err := BuildMessage(MessageInput{
		...
		CreatedAt: time.Date(2026, 3, 14, 9, 1, 0, 0, time.UTC),
	})
	if err != nil {
		t.Fatalf("BuildMessage error = %v", err)
	}
	...
	if msg.Origin.AgentID != "news/world-01" {
		t.Fatalf("agent_id = %q, want news/world-01", msg.Origin.AgentID)
	}
	if err := WriteMessage(dir, msg, body); err != nil {
		t.Fatalf("WriteMessage error = %v", err)
	}
