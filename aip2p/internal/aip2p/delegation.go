...
const DelegationVersion = "aip2p-delegation/0.1"
...
func SignWriterDelegation(parent AgentIdentity, child AgentIdentity, scopes []string, createdAt, expiresAt time.Time) (WriterDelegation, error) {
	if err := parent.ValidatePrivate(); err != nil {
		return WriterDelegation{}, err
	}
	if err := child.ValidatePrivate(); err != nil {
		return WriterDelegation{}, err
	}
	if createdAt.IsZero() {
	...
	delegation := WriterDelegation{
		...
		ParentKeyType:   strings.TrimSpace(parent.KeyType),
		ParentPublicKey: strings.ToLower(strings.TrimSpace(parent.PublicKey)),
		ChildAgentID:    strings.TrimSpace(child.AgentID),
		ChildKeyType:    strings.TrimSpace(child.KeyType),
		ChildPublicKey:  strings.ToLower(strings.TrimSpace(child.PublicKey)),
		Scopes:          uniqueFold(scopes),
		CreatedAt:       createdAt.UTC().Format(time.RFC3339),
	}
	if !expiresAt.IsZero() {
		delegation.ExpiresAt = expiresAt.UTC().Format(time.RFC3339)
	}
	payload, err := delegation.payloadBytes()
	if err != nil {
		return WriterDelegation{}, err
	}
	privateKey, err := decodeHexKey(parent.PrivateKey, ed25519.PrivateKeySize, "private_key")
	if err != nil {
		return WriterDelegation{}, err
	}
...
func SignWriterRevocation(parent AgentIdentity, childAgentID, childPublicKey, reason string, createdAt time.Time) (WriterRevocation, error) {
	if err := parent.ValidatePrivate(); err != nil {
		return WriterRevocation{}, err
	}
	if strings.TrimSpace(childAgentID) == "" {
		return WriterRevocation{}, errors.New("child_agent_id is required")
	}
	if strings.TrimSpace(childPublicKey) == "" {
		return WriterRevocation{}, errors.New("child_public_key is required")
	}
	if createdAt.IsZero() {
		createdAt = time.Now().UTC()
	}
	revocation := WriterRevocation{
		Type:            DelegationKindWriterRevocation,
		Version:         DelegationVersion,
		ParentAgentID:   strings.TrimSpace(parent.AgentID),
		ParentKeyType:   strings.TrimSpace(parent.KeyType),
		ParentPublicKey: strings.ToLower(strings.TrimSpace(parent.PublicKey)),
	...
	revocation.Signature = hex.EncodeToString(ed25519.Sign(ed25519.PrivateKey(privateKey), payload))
...
func ValidateWriterDelegation(delegation WriterDelegation) error {
	...
	if delegation.Version != DelegationVersion {
...
func ValidateWriterRevocation(revocation WriterRevocation) error {
	...
	if revocation.Version != DelegationVersion {
		return fmt.Errorf("unsupported revocation version %q", revocation.Version)
	}
	if revocation.ParentAgentID == "" || revocation.ChildAgentID == "" {
		return errors.New("parent_agent_id and child_agent_id are required")
	}
	if revocation.ParentKeyType != KeyTypeEd25519 || revocation.ChildKeyType != KeyTypeEd25519 {
...
func SaveWriterDelegation(path string, delegation WriterDelegation) error {
	...
	return writeJSONFile(path, delegation, 0o644)
}
...
func SaveWriterRevocation(path string, revocation WriterRevocation) error {
	...
	return writeJSONFile(path, revocation, 0o644)
}
