...
func RunSync(ctx context.Context, opts SyncOptions, logf func(string, ...any)) error {
	...
	if err := EnsureDefaultNetworkBootstrapConfig(opts.NetPath); err != nil {
		return fmt.Errorf("ensure network bootstrap config: %w", err)
	}
	...
	if err := ensureNetworkID(opts.NetPath, latestOrgNetworkID); err != nil {
		return fmt.Errorf("ensure aip2p.news network id: %w", err)
	}
	...
	if logf != nil {
		...
		if netCfg.Exists {
			...
			if netCfg.BitTorrentListen != "" {
				logf("configured bittorrent listen: %s", netCfg.BitTorrentListen)
			}
			if len(netCfg.LibP2PListen) > 0 {
				logf("configured libp2p listen: %s", strings.Join(netCfg.LibP2PListen, ", "))
			}
...
func (r *syncRuntime) writeStatus(ctx context.Context) error {
	...
	status.BitTorrentDHT = torrentStatus(r.torrentClient, r.netCfg.BitTorrentListen, effectiveDHTRouterCount(r.netCfg))
...
func torrentStatus(client *torrent.Client, configuredListen string, configuredRouters int) SyncBitTorrentStatus {
	status := SyncBitTorrentStatus{
		Enabled:           len(client.DhtServers()) > 0,
		ConfiguredListen:  strings.TrimSpace(configuredListen),
