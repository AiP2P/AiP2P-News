...
func startLibP2PRuntime(ctx context.Context, cfg NetworkBootstrapConfig) (*libp2pRuntime, error) {
	...
	options := []libp2p.Option{libp2p.Ping(true)}
	if len(cfg.LibP2PListen) > 0 {
		options = append(options, libp2p.ListenAddrStrings(cfg.LibP2PListen...))
	}
	...
	return &libp2pRuntime{
		...
		listen:          append([]string(nil), cfg.LibP2PListen...),
		bootstraps:      peers,
...
func (r *libp2pRuntime) Status(ctx context.Context) SyncLibP2PStatus {
	...
	status := SyncLibP2PStatus{
		...
		ConfiguredListen:     append([]string(nil), r.listen...),
